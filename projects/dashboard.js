...
function svgGroupedBarChart(el, labels, series, suffix=''){
 ...
 const formatValue=value=>suffix==='%'?Number(value).toFixed(1):Number(value).toLocaleString(undefined,{maximumFractionDigits:1});
 ...
 let bars='';labels.forEach((lab,i)=>{const start=p.l+i*group+(group-bw*series.length)/2;series.forEach((s,si)=>{const v=s.values[i],display=formatValue(v),bh=v/max*(h-p.t-p.b),x=start+si*bw,y=h-p.b-bh;bars+=`<rect tabindex="0" aria-label="${lab}, ${s.name}: ${display}${suffix}" x="${x}" y="${y}" width="${bw-3}" height="${bh}" rx="5" fill="${palette[si]}"><title>${lab} — ${s.name}: ${display}${suffix}</title></rect><text x="${x+(bw-3)/2}" y="${Math.max(p.t+10,y-5)}" text-anchor="middle" font-size="10" font-weight="700" fill="#415568">${display}${suffix}</text>`});bars+=`<text x="${p.l+i*group+group/2}" y="${h-28}" text-anchor="middle" font-size="11" fill="#667085">${lab}</text>`});
