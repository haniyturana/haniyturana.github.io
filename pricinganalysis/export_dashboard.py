...
def export_dashboard(df, output=DEFAULT_OUTPUT):
    ...
    p['gap'] = (p.benchmark-p.price)/p.benchmark
    low = p.quantity <= p.groupby('StockCode').quantity.transform('median')
    p['recommendation'] = np.select([~p.eligible | (p.orders<3), (p.gap>=.15)&low, p.gap>=.15], ['Insufficient Evidence','Review for Increase','Review Discount'],default='Maintain')
    p['reason'] = np.select([p.recommendation.eq('Insufficient Evidence'),p.recommendation.eq('Review for Increase'),p.recommendation.eq('Review Discount')], ['Fewer than 10 SKU customers, 30 SKU orders, or 3 customer-SKU orders.', 'Effective price at least 15% below median customer price; volume at or below SKU customer median.', 'Effective price at least 15% below benchmark; higher volume may explain discount.'],default='No material below-benchmark gap under the stated rules.')
    p['confidence'] = np.where(p.recommendation.eq('Insufficient Evidence'),'Low','Medium')
    ...
    for sku,g in monthly.groupby('StockCode'):
        ...
        fits.append(dict(StockCode=sku,beta=fit.slope,pvalue=fit.pvalue,r2=fit.rvalue**2,n=len(g),eligible=eligible,interpretation='Monthly observational association; seasonality, availability and customer mix are uncontrolled.',points=g[['month','price','quantity']].to_dict('records')))
        if eligible:
    ...
    fitmap = {f['StockCode']:f for f in fits}
    p['beta'] = p.StockCode.map({k:v['beta'] for k,v in fitmap.items()})
    p['new_price'] = np.where(p.recommendation.eq('Review for Increase'), np.minimum(p.price*1.05,p.benchmark), p.price)
    p['impact'] = np.nan
    model_ok=p.StockCode.isin([k for k,v in fitmap.items() if v['eligible']]) & p.recommendation.eq('Review for Increase')
    p.loc[model_ok,'impact'] = p.loc[model_ok,'revenue']*((p.loc[model_ok,'new_price']/p.loc[model_ok,'price'])**(1+p.loc[model_ok,'beta'])-1)
    quality.update(status='ready',customers=len(c),skus=len(s),identified_revenue=c.revenue.sum(),review_opportunities=int(p.recommendation.str.startswith('Review').sum()),modelled_opportunity=float(p.loc[p.impact>0,'impact'].sum()) if p.impact.notna().any() else None,elasticity_available=linregress is not None)
